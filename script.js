...
// UI তে Rate Show করার Function
function updateRateUI(rates) {
    if(document.getElementById("payoneerBuy")) document.getElementById("payoneerBuy").innerText = rates.payoneer?.buyRate || "0";
    if(document.getElementById("payoneerSell")) document.getElementById("payoneerSell").innerText = rates.payoneer?.sellRate || "0";
    if(document.getElementById("wiseBuy")) document.getElementById("wiseBuy").innerText = rates.wise?.buyRate || "0";
    if(document.getElementById("wiseSell")) document.getElementById("wiseSell").innerText = rates.wise?.sellRate || "0";
    if(document.getElementById("usdtBuy")) document.getElementById("usdtBuy").innerText = rates.usdt?.buyRate || "0";
    if(document.getElementById("usdtSell")) document.getElementById("usdtSell").innerText = rates.usdt?.sellRate || "0";
    if(document.getElementById("skrillBuy")) document.getElementById("skrillBuy").innerText = rates.skrill?.buyRate || "0";
    if(document.getElementById("skrillSell")) document.getElementById("skrillSell").innerText = rates.skrill?.sellRate || "0";
}
...
function calculateBDT() {
    ...
    if(amount > 0 && wallet && currentRates[wallet]){
        const rate = parseFloat(currentRates[wallet].buyRate);
